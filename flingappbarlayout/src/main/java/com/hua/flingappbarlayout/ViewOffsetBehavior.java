...
class ViewOffsetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    protected void layoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        calculateMaxScrollHeight(parent, (com.hua.flingappbarlayout.AppBarLayout) parent.getChildAt(0));
     ...
     * @param appBarLayout
     */
    private void calculateMaxScrollHeight(CoordinatorLayout parent, AppBarLayout appBarLayout) {
        int totalScrollRange = appBarLayout.getTotalScrollRange();
        ViewGroup viewGroup = (ViewGroup) parent.getChildAt(1);
        int bottomHeight = 0;
        if (viewGroup instanceof ScrollView || viewGroup instanceof NestedScrollView) {
            bottomHeight = viewGroup.getChildAt(0).getMeasuredHeight();
        } else {
            viewGroup.measure(0,0);
            bottomHeight = viewGroup.getMeasuredHeight() ;
        }
        maxScrollHeight = totalScrollRange + bottomHeight - parent.getMeasuredHeight();
    ...
    public boolean setTopAndBottomOffset(int offset) {
        if (maxScrollHeight <= Math.abs(offset)) {
            offset = -maxScrollHeight;
        }
        if (mViewOffsetHelper != null) {
